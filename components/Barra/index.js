...
import { View, Text, Image, StatusBar, StyleSheet, TextInput, TouchableOpacity, Alert } from 'react-native';
...
export default function Barra() {
  ...
  const { tema, corLetra, setTema, setCorLetra, fundoEscuro, setFundoEscuro } = React.useContext(TemaContext);
  ...
  function criaContato() {
    ...
    axios.get(`http://10.0.2.2:8000/usuarios/verificar-email/${email}`).then((response) => {
      if (response.data.existe) {
        ...
        const contatoExistente = contatos.find((contato) => contato.email === novoContato.email && contato.id_usuario === novoContato.id_usuario);

        if (contatoExistente) {
          Alert.alert('Erro', 'Este contato já existe.');
        } else {
          // Atualizar o estado contatos localmente
          setContatos((prevContatos) => [...prevContatos, novoContato]);

          // Adicionar o novo contato no servidor
          axios.post('http://10.0.2.2:8000/contatos', novoContato).then(() => {
  ...
  function isContatosValidos(contatosInfo) {
    return (
      contatosInfo &&
      (Array.isArray(contatosInfo) ||
        (contatosInfo.contatos && Array.isArray(contatosInfo.contatos)))
    );
  }
